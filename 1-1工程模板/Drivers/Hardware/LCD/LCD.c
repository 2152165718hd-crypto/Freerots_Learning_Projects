...
#if LCD_USE_DMA
/* DMA句柄，用于SPI1发送 (DMA2 Stream3 Channel3) */
...
#if USE_HARDWARE_SPI
    /* 硬件SPI发送，超时时间设为10ms */
    /* 单字节发送通常不用DMA，维持轮询/中断模式即可，减少DMA配置开销 */
...
static void lcd_spi_send_buffer(const uint8_t *data, size_t len)
{
    ...
    /*
     * 使用 DMA 传输
     * 注意：HAL_SPI_Transmit_DMA 也是受限于 uint16_t 的 Size，
     * 所以大于 65535 的传输同样需要拆包。
     */
    uint16_t chunk_size;
    ...
    while (len > 0)
    {
        ...
        /* 启动DMA传输 */
        if (HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)data, chunk_size) != HAL_OK)
        {
            /* 传输启动失败，回退到轮询或报错 */
        ...
        {
            /* 等待传输完成 (Blocking DMA) */
            /* 也可以使用 __WFI() 让 CPU 休眠等待中断，但需确保系统时基正常 */
...
static void lcd_gpio_init(void)
{
    ...
    */
...
#if LCD_USE_DMA
    /* ========== DMA 初始化 (DMA2 Stream3 Channel3 -> SPI1_TX) ========== */
    /* 使能DMA2时钟 */
    ...
    /* 配置DMA中断优先级并使能 */
    HAL_NVIC_SetPriority(DMA2_Stream3_IRQn, 0, 0); // 优先级可视情况调整
 ...
 */
void LCD_set_direction(st7789_dir_t dir)
{
    s_lcd.dir = dir;
...
{
    LCD_fill_rect(0, 0, s_lcd.width - 1, s_lcd.height - 1, color);
}
 ...
 *          RGB565格式：R(5bit)|G(6bit)|B(5bit)
 */
void LCD_draw_bitmap565(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *data)
{
...
void LCD_init(void)
{
    ...
    LCD_set_direction(ST7789_DIR_PORTRAIT_0);
    st7789_reg_init();
...
/* ======================== 中断服务函数 ======================== */
#if USE_HARDWARE_SPI && LCD_USE_DMA
/**
 * @brief DMA2 Stream3 中断服务函数 (SPI1 TX)
 */
 ...
 * @return 当前显示宽度，单位像素
 */
uint16_t LCD_width(void) { return s_lcd.width; }
 ...
 * @return 当前显示高度，单位像素
 */
uint16_t LCD_height(void) { return s_lcd.height; }

/* ======================== 文本与图形显示函数 ======================== */

/**
 * @function LCD_draw_char
...
void LCD_draw_string(uint16_t x, uint16_t y, const char *str, uint16_t color, uint16_t bg, uint8_t scale)
{
    while (*str)
    {
        ...
        if (bg != color)
        {
            LCD_fill_rect(x + 5 * scale, y, x + 6 * scale - 1, y + 7 * scale - 1, bg);
        }
        x += 6 * scale;
    }
}
 ...
 */
void LCD_show_int(uint16_t x, uint16_t y, int32_t num, uint8_t len, uint16_t color, uint16_t bg, uint8_t scale)
{
    char buf[16];
    char fmt[8];
    if (len > 15)
    ...
    sprintf(buf, fmt, num);
    LCD_draw_string(x, y, buf, color, bg, scale);
}

/**
 * @function LCD_show_float
 * @brief 显示带格式的浮点数
 * @param x 显示左上角X坐标
 * @param y 显示左上角Y坐标
 ...
 */
void LCD_show_float(uint16_t x, uint16_t y, float num, uint8_t len, uint8_t frac, uint16_t color, uint16_t bg, uint8_t scale)
{
    char buf[20];
    char fmt[10];
    if (len > 10)
...
/**
 * @function LCD_draw_progress_bar_border
 * @brief 绘制进度条边框(空心矩形)
 * @param x 边框左上角X坐标
 * @param y 边框左上角Y坐标
 * @param w 边框宽度(像素)
 ...
 */
void LCD_draw_progress_bar_border(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
...
/**
 * @function LCD_update_progress_bar
 ...
 *   - 数据加载等长时间操作的进度表示
 */
void LCD_update_progress_bar(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                             ...
                             uint16_t f_color, uint16_t b_color)
{
    ...
    if (now_percent > last_percent)
    {
        ...
        {
            LCD_fill_rect(inner_x + last_bar_w, inner_y,
    ...
    else
    {
        ...
        {
            LCD_fill_rect(inner_x + now_bar_w, inner_y,
...
void LCD_demo(void)
{
...
#if LCD_USE_DMA
    LCD_draw_string(10, 10, "DMA: ON", ST7789_COLOR_GREEN, ST7789_COLOR_BLACK, 2);
#else
    LCD_draw_string(10, 10, "DMA: OFF", ST7789_COLOR_RED, ST7789_COLOR_BLACK, 2);
#endif

    LCD_draw_string(10, 50, "Int:", ST7789_COLOR_WHITE, ST7789_COLOR_BLACK, 2);
    LCD_show_int(70, 50, 1024, 4, ST7789_COLOR_GREEN, ST7789_COLOR_BLACK, 2);
    LCD_draw_string(10, 80, "Flt:", ST7789_COLOR_WHITE, ST7789_COLOR_BLACK, 2);
    LCD_show_float(70, 80, 3.14159f, 1, 4, ST7789_COLOR_RED, ST7789_COLOR_BLACK, 2);

    /* 暂停2秒，让用户看清上面的文字 */
    HAL_Delay(2000);

    /* Performance Test: Clear Screen 10 times */
    /* 这会覆盖上面的文字，所以先暂停 */
    uint32_t t1 = HAL_GetTick();
    for(int i=0; i<10; i++) LCD_clear(ST7789_COLOR_BLUE);
    uint32_t t2 = HAL_GetTick();

    char buf[32];
    sprintf(buf, "10xClr: %lums", t2-t1);
    /* 在蓝屏上显示结果 */
    LCD_draw_string(10, 110, buf, ST7789_COLOR_YELLOW, ST7789_COLOR_BLUE, 2);
    ...
    LCD_draw_progress_bar_border(10, 140, 220, 20, ST7789_COLOR_BLUE);

    uint8_t current_p = 0;
    for (int i = 0; i <= 100; i++)
    {
        LCD_update_progress_bar(10, 140, 220, 20, current_p, i, 0x001F, 0x0000);
        LCD_show_int(180, 120, i, 3, 0xFFFF, 0x0000, 2);
        LCD_draw_char(220, 120, '%', 0xFFFF, 0x0000, 2);
        current_p = i;
        HAL_Delay(20);
    }
    ...
    for (int i = 0; i < 10; i++)
    {
        LCD_fill_rect(20, 180, 220, 300, colors[i]);
        HAL_Delay(100);
    }
    HAL_Delay(500);
}
