/**
 * @file LCD.h
 * @brief LCD 显示驱动接口定义头文件 (基于 ST7789 控制器)
 * @author STM32F4-HAL-Peripheral-Library
 * @version 1.0
 * @date 2024
 *
 * @details 本头文件定义了 LCD 驱动的公开 API 接口（对外调用前缀为 LCD_），
 * 包括初始化、图形绘制、文本显示、进度条等功能的函数声明和类型定义。
 *
 * @note 驱动基于 GPIO/硬件 SPI 或 软件 SPI，可通过 `USE_HARDWARE_SPI` 配置
 */
...
/**
 * @brief 是否使用DMA进行SPI传输
 * 1: 使用DMA (DMA2_Stream3)
 * 0: 使用轮询方式
 */
 ...
 * @param scale 字符缩放倍数(1~4)
 * @return 无
 * @note 例：LCD_show_int(10, 20, 42, 3, ...) 显示为"042"
 */
 ...
 * @param scale 字符缩放倍数(1~4)
 * @return 无
 * @note 例：LCD_show_float(10, 20, 3.14159, 1, 4, ...) 显示为"3.1416"
 */
 ...
 * @return 无
 * @note 通常与LCD_update_progress_bar()配合使用
 */
